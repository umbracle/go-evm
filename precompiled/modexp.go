...
func (m *ModExp) Gas(input []byte, rev evmc.Revision) uint64 {
	...
	val, tail = m.get(input, 32)
	baseLen := new(big.Int).SetBytes(val)

	val, tail = m.get(tail, 32)
	...
	if bLen := baseLen.Uint64(); bLen < uint64(len(input)) {
		val, _ = m.get(input[bLen:], int(expHeadLen))
...
func (m *ModExp) Run(input []byte) ([]byte, error) {
	...
	exponentLen, input = m.getUint64(input)
	modulusLen, input = m.getUint64(input)
	...
	val, input = m.get(input, int(baseLen))
	base := new(big.Int).SetBytes(val)

	val, input = m.get(input, int(exponentLen))
	exponent := new(big.Int).SetBytes(val)

	val, _ = m.get(input, int(modulusLen))
	...
	return m.leftPad(res, int(modulusLen)), nil
}
